...
return {
    Run = function(env, message, args, client)
        ...
        if not message.mentionedUsers.first then return "❌ You must provide a user to unmute!" end;
        local user = message.guild:getMember(message.mentionedUsers.first.id)
        if user == nil then return "❌ You must provide a user to unmute!" end;
        ...
        if author.highestRole.position <= user.highestRole.position and author.guild.owner.id ~= author.id then -- Skip the role check if the author is the owner of the server or if the author is the same role as the user
            message:reply('❌ You are unable to unmute '..user.mentionString..' as their role is higher than or equals to yours!')
            return
        end
        ...
        if message.guild.me.highestRole.position <= user.highestRole.position then -- Same as above, but for the bot (no point in attempting to kick the user if the bot is unable to do so in the first place)
            message:reply('❌ I am unable to unmute '..user.mentionString..' as their role is higher than or equals to mine!')
            return
        end
        ...
        -- Find and remove the muted role from the target user
        ...
        user:removeRole(muteroles[message.guild.id]);

        return {
            embed = {
                title = "✅ Successfully unmuted "..user.user.tag,
                fields = {
                    {name = "Unmute invoker", value = author.user.tag, inline = true},
    ...
    Name = "unmute";
    Description = "Removes the mute role from a user, if they have it.";
